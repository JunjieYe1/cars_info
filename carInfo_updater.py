...
def fetch_and_store_vehicle_tracks():
    ...
    try:
        ...
        process_new_urban_project_interface(urban_vehicles, cursor)
    ...
    for vehicle in vehicles:
        ...
        try:
            ...
            for entry in track_data:
                ...
                if last_time is None or (track_time - last_time).total_seconds() >= data_interval:
...
def process_new_urban_project_interface(vehicles, cursor):
    optimized_data = []
    ...
    for vehicle in vehicles:
        ...
        params = {
            ...
            "sessionId": "57c7ccea-5e8c-493a-8ac8-db8598deadac-02333474"  # 请使用实际的sessionId
        }

        try:
            response = requests.post("http://220.178.1.18:8542/GPSBaseserver/stdHisAlarm/getHisGPS.do", json=params)
            response.raise_for_status()
            response_data = response.json()
            if response_data['resultCode'] == 0 and response_data['data']:
                ...
                for entry in track_data:
                    ...
                    if last_time is None or (track_time - last_time).total_seconds() >= 60:
                        gcj_lng, gcj_lat = wgs84_to_gcj02(float(entry['lon']), float(entry['lat']))
                        optimized_data.append({
                            "vehicle_id": vehicle_id,
                            "latitude": gcj_lat,
                            "longitude": gcj_lng,
            ...
            log_error_details(f"新城区项目接口请求出错: {req_e}", data=params)
        except Exception as e:
            log_error_details(f"处理新城区项目数据时出错: {e}", data=params)
    ...
    if optimized_data:
        try:
            ...
            log_error_details(f"插入新城区项目轨迹数据时出错: {e}", data=optimized_data)
    else:
...
# 定时任务，每10分钟执行一次
schedule.every(10).minutes.do(fetch_and_store_vehicle_tracks)
